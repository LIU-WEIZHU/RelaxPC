<?php
    ...
    if($data != ""){
        ...
        if(isset($mydata["username"]) && isset($mydata["userpwd"]) && $mydata["username"] != "" && $mydata["userpwd"] != "" ){
            ...
            $sql = "SELECT `username`, `userpwd`, `useremail` FROM users WHERE username = '$p_Username'";
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                if(password_verify($p_Password, $row["userpwd"])){
                    ...
                    //更新uid至資料庫
                    $sql = "UPDATE users SET UID01 = '$uid' WHERE  username = '$p_Username'";
                    if(mysqli_query($conn, $sql)){
                    $sql = "SELECT username, useremail, UID01 FROM users WHERE username = '$p_Username'";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_assoc($result);
                    $mydata = array();
                    $mydata[] = $row;
